...
# Set wd
setwd("C:/Users/User/OneDrive - Tilburg University/1. Papers/Paper 3/R/24-11-06 Sim Results")
setwd("C:/Users/perezalo/OneDrive - Tilburg University/1. Papers/Paper 3/R/24-11-06 Sim Results")
...
# Extra removal for non-convergence with impossible values
idx <- which(paste(Results_final$Condition, Results_final$Replication) %in% paste(NonConverged$Condition, NonConverged$Replication))
Results_final <- Results_final[-idx, ]
...
ggplot(for_plots_long, aes(x = N_g, y = ClusRecovery, colour = Measure)) + 
  geom_line(size = 1) + 
  facet_grid(c ~ coeff) + labs(x = "Within-group Sample Size", y = "Cluster Recovery") + 
...
# Cluster Recovery 2 - NONINV
for_plots <- Results_final %>% group_by(N_g, NonInvLoadSize, c) %>% summarise(across(ARI.con:RMSE_B4.cat, mean))
for_plots_long <- pivot_longer(data = for_plots, cols = c(ARI.con, ARI.cat), names_to = "Measure",
                               values_to = "ClusRecovery") %>% dplyr::select(Measure, ClusRecovery, N_g, NonInvLoadSize, c)
...
ggplot(for_plots_long, aes(x = N_g, y = ClusRecovery, colour = Measure)) +
  ...
  ggtitle("Effects of Non-Invariance and Item Categories on Cluster Recovery")
...
for_plots <- Results_final %>% group_by(N_g, NonInvLoadSize, c) %>% summarise(across(ARI.con:RMSE.cat, mean))
for_plots_long <- pivot_longer(data = for_plots, cols = starts_with("RMSE."), # Specify the columns to pivot
                               names_to = c("Parameter", "Type"), # Use .value to split into multiple columns
                               names_pattern = "(RMSE)\\.(con|cat)",
                               values_to = "Value") %>% dplyr::select(Parameter, Type, Value, N_g, NonInvLoadSize, c)
...
ggplot(for_plots_long, aes(x = N_g, y = Value, colour = Type)) +
  ...
  scale_colour_manual(
    values = wes_palette(n = 2, name = "Darjeeling1"), # Keep your desired colours
    labels = c("RMSE_beta DWLS", "RMSE_beta ML")   # Change labels here
  ) + 
  ...
  # Add a clear plot title
  ggtitle("Effects of Non-Invariance and Item Categories on Parameter Recovery")
